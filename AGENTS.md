...
Текущая реализация - только Stage 1 foundation. Бизнес-сущности, CRUD endpoints, мобильные MVP-сценарии и работа с фото пока намеренно оставлены в TODO.
...
- Сейчас API предоставляет только `GET /health`, который проверяет, что Prisma может выполнить запрос к PostgreSQL.
- Prisma schema намеренно не содержит бизнес-моделей до Stage 2.
