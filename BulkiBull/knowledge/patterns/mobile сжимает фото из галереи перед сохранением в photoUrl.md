...
Когда у проекта еще нет backend upload, mobile может сохранить фото в существующее поле `photoUrl`, если сначала подготовить изображение на клиенте. Тот же pipeline применим и к фото из галереи, и к снимку, полученному через камеру.

## Паттерн

1. Открыть галерею телефона или камеру через `@capacitor/camera`.
2. Получить `webPath` выбранной или снятой фотографии.
...
- пользователь работает с камерой или галереей телефона, а не с ручными ссылками;
...
См. также [[фото в MVP хранится как photoUrl]] и [[desktop web использует PWA Elements для Camera.takePhoto]].
